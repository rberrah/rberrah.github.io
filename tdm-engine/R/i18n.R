...
APP_TRANSLATIONS <- c(
  ...
  "Chaque modèle analyse les mêmes données. Les prédictions sont moyennées avec des poids issus de la vraisemblance ou du critère d'Akaike. Le serveur refuse l'agrégation de modèles ne partageant pas la même molécule et la même voie d'administration." = "Each model analyzes the same data. Predictions are averaged using likelihood- or Akaike-based weights. The server rejects combinations that do not share the same drug and administration route.",
  ...
  "Le module expérimental estime directement l'AUC24 avec XGBoost à partir de profils simulés par mrgsolve, selon la méthodologie publiée pour le tacrolimus (doi:10.1016/j.phrs.2021.105578). Il utilise les concentrations et horaires récents, la dose, l'intervalle, la durée de perfusion et les covariables du modèle. Une décomposition locale DALEX explique la prédiction par rapport à un échantillon de référence entièrement synthétique." = "The experimental module estimates AUC24 directly with XGBoost from mrgsolve-simulated profiles, following the methodology published for tacrolimus (doi:10.1016/j.phrs.2021.105578). It uses recent concentrations and times, dose, interval, infusion duration, and model covariates. A local DALEX decomposition explains the prediction against a fully synthetic reference sample.",
  "Le module expérimental estime directement l'AUC24 avec XGBoost à partir de profils simulés par mrgsolve, selon la méthodologie publiée pour le tacrolimus (doi:10.1016/j.phrs.2021.105578). Il utilise les concentrations et horaires, la dose, l'intervalle, la durée de perfusion et les covariables du modèle. Une décomposition locale DALEX explique la prédiction par rapport à un échantillon de référence entièrement synthétique." = "The experimental module estimates AUC24 directly with XGBoost from mrgsolve-simulated profiles, following the methodology published for tacrolimus (doi:10.1016/j.phrs.2021.105578). It uses concentrations and times, dose, interval, infusion duration, and model covariates. A local DALEX decomposition explains the prediction against a fully synthetic reference sample.",
  "L'artefact actuellement déployé concerne le modèle Revilla de vancomycine IV intermittente et exige une administration à l'état stationnaire (ss = 1). Un seul épisode de TDM suffit s'il contient au moins deux concentrations dans un même intervalle posologique; cet intervalle n'a pas besoin d'être le plus récent. Les simulations d'entraînement répartissent les deux horaires entre 0,2 h après la dose et 0,15 h avant la dose suivante, avec au moins 0,5 h entre eux. Les deux concentrations les plus récentes de l'intervalle admissible le plus récent alimentent le prédicteur." = "The currently deployed artifact covers the Revilla intermittent-IV vancomycin model and requires an administration declared at steady state (ss = 1). One TDM occasion is sufficient when it contains at least two concentrations in the same dosing interval; that interval does not need to be the latest one. Training simulations distribute both sampling times from 0.2 hours after the dose to 0.15 hours before the next dose, with at least 0.5 hours between them. The two latest concentrations from the latest eligible interval feed the predictor.",
  "L'estimation ML est une AUC24 directe. Elle est comparée à l'AUC24 MAP du schéma répété à l'état stationnaire, et non à l'intégrale MAP historique partielle lorsque moins de 24 h sont disponibles." = "The ML estimate is a direct AUC24. It is compared with the steady-state MAP AUC24 of the repeated regimen, not with the partial historical MAP integral when fewer than 24 hours are available.",
  "Chaque artefact reste lié au même modèle PK, à la même voie, au même mode d'administration, au même schéma de variables et aux empreintes exactes du fichier mrgsolve et du booster. Il doit être favorable en validation croisée imbriquée et sur un test interne non touché; sa transportabilité vers un autre PopPK est rapportée séparément. Une variable hors des bornes empiriques d'entraînement déclenche un avertissement sans masquer l'estimation. Une donnée manquante, moins de deux concentrations dans un même intervalle, l'absence de steady state, un protocole incompatible ou une AUC invalide restent bloquants. L'estimation ML ne remplace pas les projections de dose MAP tant qu'une validation propre à la vancomycine sur patients externes n'est pas documentée." = "Each artifact remains tied to the same PK model, route, administration mode, feature schema, and exact fingerprints of the mrgsolve file and booster. It must perform favorably in nested cross-validation and on an untouched internal test set; transportability to another PopPK model is reported separately. A feature outside empirical training bounds triggers a warning without hiding the estimate. Missing data, fewer than two concentrations in one interval, no steady state, an incompatible protocol, or an invalid AUC remain blocking. ML does not replace MAP dose projections until vancomycin-specific validation in external patients is documented.",
  "Pour l'artefact actuel, la validation externe simulée sur le modèle Goti n'atteint pas les seuils préspecifiés de transportabilité. Cette limite et l'absence de validation sur des patients réels indépendants interdisent d'interpréter le résultat ML comme une validation clinique." = "For the current artifact, simulated external validation against the Goti model does not meet the prespecified transportability thresholds. This limitation and the lack of validation in independent real patients prevent interpreting the ML result as clinical validation.",
  "Chaque artefact reste lié au même modèle PK, à la même voie, au même schéma de variables et aux empreintes exactes du fichier mrgsolve et du booster. Il doit être favorable en validation croisée imbriquée et sur un test interne non touché; sa transportabilité vers un autre PopPK est rapportée séparément. Une variable hors des bornes empiriques d'entraînement est signalée comme extrapolation sans masquer l'estimation expérimentale; une donnée manquante, un protocole incompatible ou une AUC prédite invalide restent bloquants. L'estimation ML ne remplace pas les projections de dose MAP tant qu'une validation propre à la vancomycine sur patients externes n'est pas documentée." = "Each artifact remains tied to the same PK model, route, feature schema, and exact fingerprints of the mrgsolve file and booster. It must perform favorably in nested cross-validation and on an untouched internal test set; transportability to another PopPK model is reported separately. A feature outside empirical training bounds is flagged as extrapolation without hiding the experimental estimate; missing data, an incompatible protocol, or an invalid predicted AUC remain blocking. ML does not replace MAP dose projections until vancomycin-specific validation in external patients is documented.",
