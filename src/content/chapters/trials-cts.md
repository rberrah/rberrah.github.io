...
description: "Tester un essai avant de le faire : populations virtuelles, schémas posologiques et caractéristiques opératoires."
...
quiz:
  ...
  - prompt: "Pour simuler une population virtuelle réaliste, il faut..."
    ...
    correct: 0
  - prompt: "La puissance conditionnelle d'un essai simulé dépend surtout de..."
    options:
...
En répétant l'essai des milliers de fois sous un scénario fixé, on obtient la **distribution** des résultats possibles, donc une caractéristique opératoire : la puissance conditionnelle d'atteindre le critère.
<!-- /step -->
...
On estime alors des métriques : **puissance conditionnelle / caractéristique opératoire**, dose optimale, taille d'échantillon nécessaire. Le terme PoS/assurance/predictive probability est à réserver aux cadres qui intègrent explicitement l'incertitude correspondante.

$$ \widehat{Power}_{scenario} = \frac{1}{N_{sim}}\sum_{s=1}^{N_{sim}} \mathbf{1}(\text{essai } s \text{ positif}) $$
