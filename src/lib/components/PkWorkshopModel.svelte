<script>
  ...
  import { writeDraft } from '$lib/workshops/session.js';
  ...
  $effect(() => {
    ...
    if (showRegimen && model.route === 'Oral') regimen.infusion = 0;
  });
</script>
...
<style>
  .workshop-links { display: flex; flex-wrap: wrap; gap: 16px; padding: 10px 0; font-size: 0.85rem; }
