<script>
  ...
  $: thalf = Math.log(2) / ke;
  // erreur quadratique moyenne (ajustement aux vrais points)
  $: rmse = Math.sqrt(warfarinPK.reduce((s, p) => s + (p[2] - conc(p[1])) ** 2, 0) / warfarinPK.length);
  ...
  $: curve = Array.from({ length: 127 }, (_, i) => i).map((t, i) => `${i ? 'L' : 'M'}${xt(t).toFixed(1)},${yc(conc(t)).toFixed(1)}`).join(' ');
  // mode gof
...
<div class="wrap">
  <div class="controls">
    <div class="modes">
      <button class:on={mode === 'time'} on:click={() => (mode = 'time')}>Profil</button>
    ...
    <p class="hint">Points réels de la base Warfarin. Ajustez Ka, CL, V et Tlag pour rapprocher la courbe du nuage (minimiser le RMSE).</p>
  </div>
  ...
  <svg viewBox={`0 0 ${W} ${H}`} class="chart" role="img" aria-label="Ajustement du modèle aux données Warfarin réelles">
    <g transform={`translate(${m.left},${m.top})`}>
      ...
      {#if mode === 'time'}
        {#each warfarinPK as p}<circle cx={xt(p[1])} cy={yc(p[2])} r="2.4" class="pt" />{/each}
      ...
      {:else}
        ...
        {#each warfarinPK as p}<circle cx={gx(conc(p[1]))} cy={gy(p[2])} r="2.6" class="pt" />{/each}
...
<style>
  .wrap { display: grid; gap: var(--space-4); --pk: var(--accent-pk); }
